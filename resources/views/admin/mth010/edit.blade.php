...
<br>
{!! Form::model($mth010, array('route' => array('mth010.update', $mth010->id), 'method' => 'PUT')) !!}
    ...
    <div class="col-sm-6">
        ...
        <h4>Last Created:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($mth010->created_at)) }}</p>
         
        </div>
        
        <div class="col-sm-6">
       <h4> Last Updated:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($mth010->updated_at)) }}</p>
        </div>   
    </div> 
    <br>
    ...
    <div class="col-sm-3">
        ...
        {!! Html::linkRoute('mth010.index', 'Cancel', array($mth010->id), array('class'=>'btn btn-danger btn-block')) !!}
