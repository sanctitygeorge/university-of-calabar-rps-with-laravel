...
<br>
{!! Form::model($bio020, array('route' => array('bio020.update', $bio020->id), 'method' => 'PUT')) !!}
    ...
    <div class="col-sm-6">
        ...
        <h4>Last Created:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($bio020->created_at)) }}</p>
         
        </div>
        
        <div class="col-sm-6">
       <h4> Last Updated:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($bio020->updated_at)) }}</p>
        </div>   
    </div> 
    <br>
    ...
    <div class="col-sm-3">
        ...
        {!! Html::linkRoute('bio020.index', 'Cancel', array($bio020->id), array('class'=>'btn btn-danger btn-block')) !!}
