...
<br>
{!! Form::model($mth020, array('route' => array('mth020.update', $mth020->id), 'method' => 'PUT')) !!}
    ...
    <div class="col-sm-6">
        ...
        <h4>Last Created:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($mth020->created_at)) }}</p>
         
        </div>
        
        <div class="col-sm-6">
       <h4> Last Updated:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($mth020->updated_at)) }}</p>
        </div>   
    </div> 
    <br>
    ...
    <div class="col-sm-3">
        ...
        {!! Html::linkRoute('mth020.index', 'Cancel', array($mth020->id), array('class'=>'btn btn-danger btn-block')) !!}
