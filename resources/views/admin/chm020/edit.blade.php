...
<br>
{!! Form::model($chm020, array('route' => array('chm020.update', $chm020->id), 'method' => 'PUT')) !!}
    ...
    <div class="col-sm-6">
        ...
        <h4>Last Created:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($chm020->created_at)) }}</p>
         
        </div>
        
        <div class="col-sm-6">
       <h4> Last Updated:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($chm020->updated_at)) }}</p>
        </div>   
    </div> 
    <br>
    ...
    <div class="col-sm-3">
        ...
        {!! Html::linkRoute('chm020.index', 'Cancel', array($chm020->id), array('class'=>'btn btn-danger btn-block')) !!}
