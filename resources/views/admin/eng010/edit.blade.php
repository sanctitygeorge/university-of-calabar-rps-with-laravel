...
{!! Form::model($eng010, array('route' => array('eng010.update', $eng010->id), 'method' => 'PUT')) !!}
    ...
    <div class="col-sm-6">
        ...
        <h4>Last Created:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($eng010->created_at)) }}</p>
         
        </div>
        
        <div class="col-sm-6">
       <h4> Last Updated:</h4> <p class="lead"> {{ date('M j, Y h:ia', strtotime($eng010->updated_at)) }}</p>
        </div>   
    </div> 
    <br>
    ...
    <div class="col-sm-3">
        ...
        {!! Html::linkRoute('eng010.index', 'Cancel', array($eng010->id), array('class'=>'btn btn-danger btn-block')) !!}
